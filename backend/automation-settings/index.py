        ...
        if method == 'GET':
            ...
            # Daily Analytics
                ...
                job_id = create_cronjob(
                    ...
                    url='https://functions.poehali.dev/0aa16e4e-addc-4138-8634-608c908f3fab',
                    hours=[9],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'daily_analytics', job_id)
                conn.commit()
            ...
            # Error Monitoring
                ...
                job_id = create_cronjob(
                    ...
                    url='https://functions.poehali.dev/a617b156-a565-4956-aef0-6bfcc40fbc9b',
                    hours=[0, 6, 12, 18],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'error_monitoring', job_id)
                conn.commit()
                result = {'ok': True, 'message': 'Мониторинг ошибок включён (каждые 6 часов)'}
            ...
            # Cleanup Logs
                ...
                job_id = create_cronjob(
                    api_key=api_key,
                    title='Очистка старых логов (30+ дней)',
                    url='https://functions.poehali.dev/495bb14f-1eee-4584-8c20-12d7957a9ede',
                    hours=[4],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'cleanup_logs', job_id)
                conn.commit()
            ...
            # Subscription Reminders
                ...
                job_id = create_cronjob(
                    api_key=api_key,
                    title='Напоминания о подписке',
                    url='https://functions.poehali.dev/9f82606a-d539-46e2-a654-943dea13e62b',
                    hours=[10],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'subscription_reminders', job_id)
                conn.commit()
                result = {'ok': True, 'message': 'Напоминания о подписке включены'}
                ...
                if job_id:
                    disable_cronjob(api_key, job_id)
                result = {'ok': True, 'message': 'Напоминания о подписке отключены'}
            ...
            # Admin Alerts
                ...
                job_id = create_cronjob(
                    api_key=api_key,
                    title='Уведомления админам о событиях',
                    url='https://functions.poehali.dev/bdcff42e-39cd-4fc4-a8b0-f94306385205',
                    hours=[8, 20],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'admin_alerts', job_id)
                conn.commit()
                result = {'ok': True, 'message': 'Уведомления админам включены (2 раза в день)'}
                ...
                if job_id:
                    disable_cronjob(api_key, job_id)
                result = {'ok': True, 'message': 'Уведомления админам отключены'}
            ...
            # Security Audit
                ...
                job_id = create_cronjob(
                    api_key=api_key,
                    title='Аудит безопасности системы',
                    url='https://functions.poehali.dev/cdbd4850-9224-49a7-8614-27bce0625b39',
                    hours=[1, 13],
                    enabled=True
                )
                save_cronjob_id(conn, cur, 'security_audit', job_id)
                conn.commit()
                result = {'ok': True, 'message': 'Аудит безопасности включён (2 раза в день)'}
